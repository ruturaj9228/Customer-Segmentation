...
const InsightsCards = ({ insights, selectedColumns, selectedCluster, onSelectCluster }) => {
  if (!insights || insights.length === 0) return null;

  return (
    <>
      {insights.map((insight, idx) => {
        const isSelected = selectedCluster === String(insight.cluster);
        return (
            <motion.div 
              key={idx}
              onClick={() => onSelectCluster(String(insight.cluster))}
              className={`glass-panel group relative overflow-hidden cursor-pointer transition-all ${isSelected ? 'ring-2 ring-white/50 scale-[1.02]' : 'hover:ring-1 hover:ring-white/20'}`}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4, delay: idx * 0.1 }}
              whileHover={{ y: -5 }}
              style={{
                  boxShadow: isSelected ? `0 0 30px ${CLUSTER_COLORS[insight.cluster % CLUSTER_COLORS.length]}33` : undefined
              }}
            >
                  ...
                  <div className="grid gap-3">
                      ...
                      {selectedColumns.length > 5 && (
                          <div className="text-xs text-primary text-center italic mt-2">
                              +{selectedColumns.length - 5} more metrics available
                          </div>
                      )}
                  </div>
                </div>
                
                <div className="mt-6 pt-4 border-t border-white/5 text-center transition-colors">
                    <span className={`text-sm font-medium ${isSelected ? 'text-primary' : 'text-muted group-hover:text-white'}`}>
                        {isSelected ? 'Currently Viewing Details' : 'Click to View Details'}
                    </span>
                </div>
              </div>
            </motion.div>
